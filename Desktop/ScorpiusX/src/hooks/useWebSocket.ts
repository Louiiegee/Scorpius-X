  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        // Only reconnect if user is authenticated, connection was intentional, and we haven't exceeded max attempts
        if (user && event.code !== 1006 && reconnectCountRef.current < MAX_RECONNECT_ATTEMPTS) {
          setTimeout(() => {
            reconnectCountRef.current++;
            connect();
          }, RECONNECT_DELAY);
        } else if (event.code === 1006) {
          console.warn('WebSocket backend server not available - skipping reconnection attempts');
        }
      };
        }
      };
      ...
      ws.onerror = (error) => {
        console.warn('WebSocket connection failed - backend server not available');
        setIsConnected(false);
      };
