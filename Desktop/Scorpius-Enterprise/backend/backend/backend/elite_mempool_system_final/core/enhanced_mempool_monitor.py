...
from .utils import async_retry, ether_to_wei, wei_to_ether
        ...
        for url in self.rpc_urls:
            try:
                if url.startswith("ws"):
                    provider = WebsocketProviderV2(url, request_timeout=self.request_timeout_for_provider)
                else: # HTTP
        ...
        while not self._shutting_down:
            try:
                ...
                if not self._pending_filter_id:
                    try:
                        self._pending_filter_id = await self._active_web3.eth.new_pending_transaction_filter()
                        logger.info(f"New filter: {self._pending_filter_id} on {getattr(self._active_web3.provider, 'endpoint_uri', 'Unknown')}")
                    except Exception as e:
                ...
                pending_tx_hashes_raw = await self._active_web3.eth.get_filter_changes(self._pending_filter_id)
