...
const SCANNER_API_URL = "http://localhost:8001/api";
const MEMPOOL_API_URL = "http://localhost:8002/api"; 
const MEV_API_URL = "http://localhost:8003/api";

// Legacy fallback for dashboard/auth endpoints
...
// Enhanced API request helper that routes to correct microservice
const apiRequest = async (endpoint: string, options: RequestInit = {}) => {
  // Route to appropriate microservice based on endpoint
  let baseUrl = API_BASE_URL;
  
  if (endpoint.startsWith('/scanner')) {
    baseUrl = SCANNER_API_URL;
  } else if (endpoint.startsWith('/mempool')) {
    baseUrl = MEMPOOL_API_URL;
  } else if (endpoint.startsWith('/mev')) {
    baseUrl = MEV_API_URL;
  }
...
export const scannerAPI = {
  ...
  uploadContract: async (file: File) => {
    ...
    const token = localStorage.getItem("scorpius_token");
    const response = await fetch(`${SCANNER_API_URL}/scanner/upload`, {
      method: "POST",
      headers: {
        ...(token && { Authorization: `Bearer ${token}` }),
      },
